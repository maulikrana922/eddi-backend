        ...
        if uuid:
            ...
            if email_id:
                if getattr(models,USERSIGNUP_TABLE).objects.get(**{EMAIL_ID:email_id}).user_type.user_type == SUPPLIER_S:
                    ...
                    data_category = getattr(models,COURSEDETAILS_TABLE).objects.filter(Q(organization_domain = organization_domain) | Q(course_category__category_name__in = a)).filter(**{STATUS_ID:1, IS_APPROVED_ID:1}).exclude().order_by("-organization_domain")
    ...
    def put(self, request, uuid=None):
        ...
        try:
            ...
            if request.FILES.getlist(DOCUMENT_FILES):
                ...
                for i in document_files:
                    # l = []
                    # data1 = getattr(models,"MaterialDocumentMaterial").objects.get(**{"document_file":i})
                    a = course_material_data.document_files.all()
                    l = [i.document_file for i in a]
                    print(l, "aaaaaa")

                    # a = getattr(models,"CourseMaterial").objects.get(**{"course__uuid":uuid}).value()
                    # print(a, "aaaaa")
                    # if data1 in course_material_data.document_files.all().values_list("document_files", flat=True):
                    #     pass
